...
因为ca是可信的, 操作系统默认内置了很多ca机构(根机构,中间机构)的证书, 找一下有没有熟悉的, 如下:
...
4. ca 合并 ca_pubkey+ca_info+encode_ca_hash 生成自签名证书ca.crt, 它可以签署下一级证书
```
...
2. ca2 把 ca2_pubkey+ca2基本信息ca2_info 送到根机构ca
3. ca 验证ca2身份, 再附上自己信息ca_info, 然后把 ca2_pubkey+ca2_info+ca_info 合并进行摘要, 得到ca2_hash
4. ca 用自己的ca_prikey对ca2_hash加密, 得到指纹 encode_ca2_hash
5. ca 合并 ca2_pubkey+ca2_info+ca_info+encode_ca2_hash 生成签名数字证书ca2.crt, 它也可以签署下一级证书
```
...
4. ca2 把 s2_pubkey+s2_info+ca2_info 合并进行摘要, 得到s2_hash
5. ca2 用自己的ca2_prikey对s2_hash加密, 得到指纹encode_s2_hash
6. ca2 合并 s2_pubkey+s2_info+ca2_info+encode_s2_hash 生成签名数字证书s2.crt, 它不能用于签署下级证书.
